...
//var targetRotationVector = [0, 0, 0]; //saves the target vector to rotate to
var degPerFrame = 1;
var degNeededToRotate = 0; //signed
var cumulativeDeg = 0; 

var mxMax = window.visualViewport.width/2;
var myMax = window.visualViewport.height/2;
...
function init() {
    ...
    window.onmousemove = (event) => {
        //console.log(event);
        mx = event.x - window.visualViewport.width/2 //recenter to have 0 0 be the center of the screen
        if (mx == 0) return;
        ...
        // degNeededToRotate = degBetween2Vector([mx, 0, -1], [0, 0, -1]) * Math.sign(mx);
        // if(degNeededToRotate < degPerFrame) degNeededToRotate = 0 
        // cumulativeDeg = 0;
    }
    ...
    setInterval(() => {
        ...
        // var deg = 2.6
        if(v2[0] != 0){ //arrow rotation
            //d += deg * v2[0]
            rotateScene(sceneData, [0, -1, 0], [0, 0, 0], degPerFrame * v2[0])
            //var k = rotateAroundLine(sceneData.respectiveCamNormal, [0, -1, 0], [0, 0, 0], -degPerFrame * v2[0])
            //sceneData.respectiveCamNormal = k
        }
        
        else if(degNeededToRotate != 0 &&
            !(degNeededToRotate > 0 && cumulativeDeg <= degNeededToRotate && cumulativeDeg + degPerFrame >= degNeededToRotate ||
            degNeededToRotate < 0 && cumulativeDeg >= degNeededToRotate && cumulativeDeg + degPerFrame <= degNeededToRotate)
        ){
            rotateScene(sceneData, [0, -1, 0], [0, 0, 0], degPerFrame * Math.sign(degNeededToRotate));
            cumulativeDeg += degPerFrame * Math.sign(degNeededToRotate);
        }
...
function rotate2d(point, pivot, angle) {
    ...
    var a = x_pivot + (x_shifted * c - y_shifted * s);
    var b = y_pivot + (x_shifted * s + y_shifted * c);
